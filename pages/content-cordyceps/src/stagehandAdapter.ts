 ...
 * Cordyceps HandledInjectedScript system in our Chrome extension environment.
 *
...
import type { HandledInjectedScript } from './handledInjectedScript';
...
export class StagehandCordycepsAdapter {
  private handledInjectedScript: HandledInjectedScript;
  private config: Required<StagehandAdapterConfig>;
  private isInitialized: boolean = false;

  constructor(handledInjectedScript: HandledInjectedScript, config: StagehandAdapterConfig = {}) {
    this.handledInjectedScript = handledInjectedScript;
    this.config = {
  ...
  async generateXPathsForElement(element: Element): Promise<XPathGenerationResult> {
    ...
    try {
      handle = this.handledInjectedScript.getHandleForElement(element);
    } catch (error) {
      if (this.config.debug) {
  ...
  async getElementByXPathWithHandle(
    ...
    try {
      const handle = this.handledInjectedScript.getHandleForElement(element);
  ...
  async interactWithElementByXPath(
    ...
    try {
      switch (action) {
        case 'click': {
          const clickResult = this.handledInjectedScript.clickElementWithOptions(handle, options);
          return { ...clickResult, handle };
        }
        ...
        case 'type': {
          ...
          // Use the underlying InjectedScript's fill method which handles text input
          const fillResult = this.handledInjectedScript.injectedScript.fill(element, options.text);
          if (fillResult === 'error:notconnected') {
            return { success: false, error: 'Element is not connected to the DOM', handle };
          } else if (fillResult === 'needsinput') {
            return {
              success: true,
              error: 'Element filled, may need additional input events',
              handle,
            };
          } else {
  ...
  getStatus() {
    return {
      ...
      config: this.config,
      cordycepsHandlesCount: this.handledInjectedScript.handleManager.cacheSize,
    };
  }
...
export function createStagehandAdapter(
  handledInjectedScript?: HandledInjectedScript,
  config?: StagehandAdapterConfig
): StagehandCordycepsAdapter {
  const injectedScript = handledInjectedScript || window.__cordyceps_handledInjectedScript;

  if (!injectedScript) {
    throw new Error(
      'HandledInjectedScript not available. Ensure Cordyceps is properly initialized.'
    );
  }

  return new StagehandCordycepsAdapter(injectedScript, config);
}
