...
export const operatorResponseSchema = z.object({
  reasoning: z
    .string()
    .describe(
      "The reasoning for the step taken. If this step's method is `close`, the goal was to " +
        'extract data, and the task was successful, state the data that was extracted.'
    ),
  ...
  parameters: z
    ...
    .describe(
      `The parameter for the action. Only pass in a parameter for the following methods:
        - act: The action to perform. e.g. "click on the submit button" or
          "type [email] into the email input field and press enter"
        - extract: The data to extract. e.g. "the title of the article". If you
          want to extract all of the text on the page, leave this undefined.
  ...
  taskComplete: z
    .boolean()
    .describe(
      'Whether the task is complete. If true, the task is complete and no more steps are needed. ' +
        'If you chose to close the task because the goal is not achievable, set this to false.'
    ),
});
