...
import { GotoOptions } from '../../playwright';
...
async function getBrowser(
  ...
  if (env === 'BROWSERBASE') {
    ...
    if (!projectId) {
      throw new MissingEnvironmentVariableError('BROWSERBASE_PROJECT_ID', 'Browserbase');
    }
    ...
    if (browserbaseSessionID) {
      ...
      try {
        const session = await browserbase.sessions.retrieve(browserbaseSessionID);
    ...
    if (localBrowserLaunchOptions?.cdpUrl) {
      ...
      const browser = await chromium.connectOverCDP(localBrowserLaunchOptions.cdpUrl);
      const context = browser.contexts()[0];
    ...
    const context = await chromium.launchPersistentContext(userDataDir, {
      ...
      args: localBrowserLaunchOptions?.args ?? ['--disable-blink-features=AutomationControlled'],
...
export class Stagehand {
  ...
  constructor(
    ...
    this.enableCaching =
      enableCaching ?? (process.env.ENABLE_CACHING && process.env.ENABLE_CACHING === 'true');
  ...
  private registerSignalHandlers() {
    const cleanup = async (signal: string) => {
      ...
      this.stagehandLogger.info(`[${signal}] received. Ending Browserbase session...`);
      try {
        await this.close();
      } catch (err) {
  ...
  public get downloadsPath(): string {
    return this.env === 'BROWSERBASE'
      ? 'downloads'
      : (this.localBrowserLaunchOptions?.downloadsPath ?? path.resolve(process.cwd(), 'downloads'));
  }
  ...
  async init(): Promise<InitResult> {
    ...
    const { browser, context, debugUrl, sessionUrl, contextPath, sessionId } = await getBrowser(
      this.apiKey,
      this.projectId,
      this.env,
      this.headless,
...
`;
  ...
  async close(): Promise<void> {
    ...
    if (this.contextPath && !this.localBrowserLaunchOptions?.preserveUserDataDir) {
      try {
