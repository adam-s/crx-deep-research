import { AnthropicMessage, AnthropicContentBlock, AnthropicToolResult } from '../../../types/agent';
...
export function compressConversationImages(
  ...
  items.forEach((item, index) => {
    ...
    const shouldCompress =
      imageIndex >= 0 && imageIndex < itemsWithImages.length - keepMostRecentCount;

    if (shouldCompress) {
      if (Array.isArray(item.content)) {
        item.content = item.content.map((contentItem: AnthropicContentBlock) => {
          if (
            contentItem.type === 'tool_result' &&
            'content' in contentItem &&
            Array.isArray(contentItem.content) &&
