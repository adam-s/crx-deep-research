...
import { createOpenAIInitialInputItems, safeUsageAccess } from '../../types/agentUtils';
...
export class OpenAICUAClient extends AgentClient {
  ...
  private createInitialInputItems(instruction: string): ResponseInputItem[] {
    // Use the utility function to safely handle userProvidedInstructions
    return createOpenAIInitialInputItems(instruction, this.userProvidedInstructions);
  }
  ...
  async getAction(
    ...
    try {
      ...
      // Extract only the input_tokens and output_tokens safely
      const safeUsage = safeUsageAccess(response.usage);
      const usage = {
  ...
  private convertComputerCallToAction(call: ComputerCallItem): AgentAction | null {
    ...
    // The action already contains the type property, so we don't need to set it explicitly
    return {
      ...action, // Spread all properties from the action (including type)
    } as AgentAction;
  }
