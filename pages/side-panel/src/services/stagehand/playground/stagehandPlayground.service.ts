...
// COMMENTED OUT IMPORTS - keeping for later when we re-enable other tests
...
import { testActHandlerUtils } from './playgroundTests/actHandlerUtilsTests';
...
export class StagehandPlaygroundService extends Disposable implements IStagehandPlaygroundService {
  ...
  public async runAllTests(): Promise<void> {
    ...
    try {
      ...
      // COMMENTED OUT - PREVIOUS TESTS (keeping for later)
  ...
  public async runDOMUtilsTests(): Promise<void> {
    this.events.emit({
      ...
      severity: Severity.Info,
      message:
        '🔧 [COMMENTED OUT] Testing Stagehand DOM utilities and pure functions (15% - no conversion needed)...',
    });

    // COMMENTED OUT - keeping for later
    // try {
  ...
  public async runCordycepsConversionTests(): Promise<void> {
    this.events.emit({
      timestamp: Date.now(),
      severity: Severity.Info,
      message: '🔄 [COMMENTED OUT] Testing Stagehand -> Cordyceps API conversions (60%)...',
    });
  ...
  public async runLivePageDOMTests(): Promise<void> {
    this.events.emit({
      timestamp: Date.now(),
      severity: Severity.Info,
      message: '🌐 [COMMENTED OUT] Testing with live page at http://localhost:3005...',
    });
  ...
  public async runQuickTests(): Promise<boolean> {
    ...
    try {
      ...
      // COMMENTED OUT - PREVIOUS QUICK TESTS (keeping for later)
  ...
  public async runEasyConversionTests(): Promise<void> {
    this.events.emit({
      timestamp: Date.now(),
      severity: Severity.Info,
      message: '🔧 [COMMENTED OUT] Running easy conversion category tests...',
      details: {
        categories: [
  ...
  public async runHandlerTests(): Promise<void> {
    ...
    try {
      ...
      // Test ActHandlerUtils
      this.events.emit({
        timestamp: Date.now(),
        severity: Severity.Info,
        message: '🔧 Testing ActHandlerUtils...',
      });
      await testActHandlerUtils(testContext);
