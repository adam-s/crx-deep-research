...
export function parseCSS(
  ...
  try {
    ...
    if (e instanceof Error) {
      const newMessage =
        e.message + ` while parsing css selector "${selector}". Did you mean to CSS.escape it?`;
      const stack = e.stack;
      const index = stack ? stack.indexOf(e.message) : -1;
      if (index !== -1 && stack) {
        e.stack =
          stack.substring(0, index) + newMessage + stack.substring(index + e.message.length);
      }
      ...
      throw new Error(
        errorMessage + ` while parsing css selector "${selector}". Did you mean to CSS.escape it?`,
      );
    }
  }
