...
export class StorageScript {
  ...
  private _isPlainObject(v: any) {
    ...
    if (this._isFirefox) {
      ...
      if (
        constructorImpl?.startsWith('function Object() {') &&
        constructorImpl?.includes('[native code]')
      ) {
        return true;
      }
    }
  ...
  private async _collectDB(dbInfo: IDBDatabaseInfo) {
    ...
    const stores = await Promise.all(
      [...db.objectStoreNames].map(async storeName => {
  ...
  async collect(recordIndexedDB: boolean): Promise<SerializedStorage> {
    const localStorage = Object.keys(this._global.localStorage).map(name => ({
      name,
      value: this._global.localStorage.getItem(name)!,
    }));
    if (!recordIndexedDB) {
      return { localStorage };
    }
    try {
      ...
      throw new Error('Unable to serialize IndexedDB: ' + errorMessage);
    }
  }
  ...
  private async _restoreDB(dbInfo: IndexedDBDatabase) {
    ...
    openRequest.addEventListener('upgradeneeded', () => {
      ...
      for (const store of dbInfo.stores) {
        const objectStore = db.createObjectStore(store.name, {
          ...
          keyPath: store.keyPathArray ?? store.keyPath,
        });
        for (const index of store.indexes) {
          objectStore.createIndex(index.name, index.keyPathArray ?? index.keyPath!, {
            unique: index.unique,
            multiEntry: index.multiEntry,
          });
        }
    ...
    await Promise.all(
      dbInfo.stores.map(async store => {
        const objectStore = transaction.objectStore(store.name);
        await Promise.all(
          store.records.map(async record => {
  ...
  async restore(originState: SetOriginStorage) {
    try {
      await Promise.all((originState.indexedDB ?? []).map(dbInfo => this._restoreDB(dbInfo)));
    } catch (e) {
      const errorMessage = e instanceof Error ? e.message : String(e);
      throw new Error('Unable to restore IndexedDB: ' + errorMessage);
    }
