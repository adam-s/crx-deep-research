...
class InspectTool implements RecorderTool {
  ...
  onClick(event: MouseEvent) {
    ...
    if (this._hoveredModel?.selector) this._commit(this._hoveredModel.selector, this._hoveredModel);
  }
...
class RecordActionTool implements RecorderTool {
  ...
  private _commitPendingClickAction() {
    if (this._pendingClickAction) this._performAction(this._pendingClickAction.action);
    this._cancelPendingClickAction();
  }
  ...
  private _onFocus(userGesture: boolean) {
    ...
    const result = activeElement
      ...
      : null;
    this._activeModel =
      result && result.selector ? { ...result, color: HighlightColors.action } : null;
    if (userGesture) {
  ...
  private _actionInProgress(event: Event): boolean {
    ...
    for (const action of this._performingActions) {
      if (isKeyEvent && action.name === 'press' && event.key === action.key) return true;
      if (
  ...
  private _updateModelForHoveredElement() {
    ...
    const { selector, elements } = this._recorder.injectedScript.generateSelector(
...
class JsonRecordActionTool implements RecorderTool {
  ...
  onInput(event: Event) {
    ...
    if (['INPUT', 'TEXTAREA'].includes(element.nodeName) || element.isContentEditable) {
      if (
        ...
        ['checkbox', 'radio'].includes((element as HTMLInputElement).type.toLowerCase())
      ) {
        // Checkbox is handled in click, we can't let input trigger on checkbox - that would mean we dispatched click events while recording.
        return;
      }
  ...
  private _isEditable(element: HTMLElement) {
    if (element.nodeName === 'TEXTAREA' || element.nodeName === 'INPUT') return true;
...
class TextAssertionTool implements RecorderTool {
  ...
  onMouseMove(event: MouseEvent) {
    ...
    if (this._kind === 'text' || this._kind === 'snapshot') {
      this._hoverHighlight = this._recorder.injectedScript.utils.elementText(
        ...
        ? { elements: [target], selector: '', color: HighlightColors.assert }
        : null;
  ...
  private _elementHasValue(element: Element) {
    return (
      ...
      (element.nodeName === 'INPUT' &&
        !['button', 'image', 'reset', 'submit'].includes((element as HTMLInputElement).type))
    );
  }
...
class Overlay {
  ...
  setUIState(state: UIState) {
    ...
    this._assertVisibilityToggle.classList.toggle(
      'disabled',
      state.mode === 'none' || state.mode === 'standby' || state.mode === 'inspecting',
    );
...
export class Recorder {
  ...
  setUIState(state: UIState, delegate: RecorderDelegate) {
    ...
    if (this._lastHighlightedAriaTemplateJSON !== ariaTemplateJSON) {
      const elements = state.ariaTemplate
        ? this.injectedScript.getAllByAria(this.document, state.ariaTemplate)
        : [];
      if (elements.length) {
  ...
  private _updateHighlight(model: HighlightModel | null, userGesture: boolean) {
    ...
    if (model)
      this.highlight.updateHighlight(
        model.elements.map(element => ({ element, color: model.color, tooltipText })),
      );
  ...
  deepEventTarget(event: Event): HTMLElement {
    for (const element of event.composedPath()) {
      if (!this.overlay?.contains(element as Element)) return element as HTMLElement;
    }
...
class Dialog {
  ...
  show(options: { label: string; body: Element; onCommit: () => void; onCancel?: () => void }) {
