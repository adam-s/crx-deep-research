...
import * as css from './utils/isomorphic/cssTokenizer';
...
import type { AriaRole } from './utils/isomorphic/ariaSnapshot';
  ...
  if (style?.display === 'contents' && !isSlot) {
    ...
    for (let child = element.firstChild; child; child = child.nextSibling) {
      if (
        ...
        !isElementHiddenForAria(child as Element)
      ) {
        return false;
      }
...
function isNativelyDisabled(element: Element) {
  ...
  return (
    ...
    (element.hasAttribute('disabled') ||
      belongsToDisabledOptGroup(element) ||
