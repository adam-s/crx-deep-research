...
export function parseCSS(
  ...
  try {
    ...
    const newMessage =
      error.message + ` while parsing css selector "${selector}". Did you mean to CSS.escape it?`;
    const stack = error.stack ?? '';
    const index = stack.indexOf(error.message);
    if (index !== -1) {
      error.stack =
        stack.substring(0, index) + newMessage + stack.substring(index + error.message.length);
    }
